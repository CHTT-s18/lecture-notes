...
particular it is necessary to show that $\textsc{Fun}(\Phi)\subseteq\Phi$.  Suppose that
$\textsc{Fun}(\Phi)(A_{0},A_{0}',\phi)$, Then by definition
...
\item if $M\sim M'\in\phi_{1}$, then $A_{2}[M/a]\sim A_{2}'[M'/a]\downarrow\phi_{2}[M,M']\in\Phi$.
\end{enumerate}
These conditions imply that $\phi_{1}$ is a PER, and that
$A'_{2}[M'/a]\sim A_{2}[M/a]\downarrow \phi_{2}[M,M']\in\mu$, with $\phi_{2}[M,M']$ being a PER, whenever
$M\sim M'\in\phi_{1}$.
% and $\phi[M,M']$ is a PER.
...
\item if $M\sim M'\in\phi_{1}$, then $A_{2}'[M/a]\sim A_{2}[M'/a]\downarrow\phi_{2}[M,M']\in\mu$.
\end{enumerate}
The first of these follows directly from the assumption
$A_{1}\sim A_{1}'\downarrow\phi_{1}\in\Phi$.  For the second, suppose that
$M\sim M'\in\phi_{1}$, with the intent to show that
$A_{2}'[M/a]\sim A_{2}[M'/a]\downarrow \phi_{2}[M,M']\in\mu$.  Because $\phi_{1}$ is symmetric,
$M'\sim M\in\phi_{1}$, and so by the second assumption
$A_{2}[M'/a] \sim A_{2}'[M/a]\downarrow\phi_{2}[M',M]\in\mu$ and $\phi_{2}[M',M]$ is a PER.  But then by
symmetry $A_{2}'[M/a]\sim A_{2}[M'/a]\downarrow\phi_{2}[M',M]\in\mu$.  The proof of this case would be
complete if $\phi_{2}[M',M]=\phi_{2}[M,M']$.
