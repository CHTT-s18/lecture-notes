...
\newcommand{\sortclass}{\ensuremath{\mathsf{sort}}}
...
\begin{figure}[tp]
  ...
  \begin{tabular}{l@{\qquad}l}
    ...
    $\eqctx{\Gamma}{\Gamma'}$ & $\Gamma$ and $\Gamma'$ are equal contexts \\
...
judgment forms of \LF{} to a particular signature $\Sigma$ are defined in Figure~\ref{fig:lf-specialized}.

\begin{figure}[tp]
  \centering
  \begin{tabular}{l@{\quad\textit{means}\quad}l}
    $\issig{\Sigma}$             & $\isctx{\Sigma}$ \\[1ex]

    $\isctx[\Sigma]{\Gamma}$           & $\isctx{\Sigma\,\Gamma}$  \\
    $\iscls[\Sigma]{\Gamma}{K}$           & $\iscls{\Sigma\,\Gamma}{K}$ \\
    $\isobj[\Sigma]{\Gamma}{O}{K}$     & $\isobj{\Sigma\,\Gamma}{O}{K}$ \\[1ex]

    $\eqctx[\Sigma]{\Gamma}{\Gamma'}$     & $\eqctx{\Sigma\,\Gamma}{\Sigma\,\Gamma'}$ \\
