...
satisfies the equation $\mu(\Delta_{H})=H(\mu(\Delta_{H}),\mu(\Delta_{H}))$, the simultaneous fixed point of
...
$\mu(H^{\mu(\Delta_{H})})\subseteq\Delta_{H}$:
\begin{align*}
...
$\Delta_{H}(\mu(A\mapsto\mu(H^{A})))\subset \mu(A\mapsto\mu(H^{A}))$.  Writing the left-hand side as $\mu(H')$, calculate
\begin{align*}
  \Delta_{H}(\mu(H'))
  & = H(\mu(H'),\mu(H')) \\
  & = H(\mu(H'),H'(\mu(H'))) \\
