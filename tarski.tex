...
$F:\wp{X}\times\wp{X}\to\wp{X}$ is monotone in each argument.  Define
$\Delta_{F}(A)=F(A,A)$, the diagonalization of $F$.  Let
$L_{F}(B) \eqdef{} \mu(A\mapsto F(A,B))$ and $R_{F}(A) \eqdef{} \mu(B\mapsto F(A,B))$.  The Diagonal Lemma
states that the simultaneous and iterated fixed points coincide,
$\mu(\Delta_{F})=\mu(L_{F})=\mu(R_{F})$.  Calculating,
\begin{align*}
  \mu(L_{F}) & = L_{F}(\mu(L_{F})) \\
           & = \mu(A\mapsto F(A,\mu(L_{F}))) \\
           & = F(\mu(L_{F}),\mu(L_{F})) \\
           & = \Delta_{F}(\mu(L_{F})). 
\end{align*}
Thus, in particular, $\mu(L_{F})$ is a pre-fixed point of $\Delta_{F}$, and hence contains
$\mu(\Delta_{F})$.  The proof that $\mu(\Delta_{F}) = \mu(R_{F})$ is analogous, establishing the Diagonal

To prove Bekic's Lemma, simply apply the Diagonal Lemma to $\Delta_{(F,G)}$, and note that
$\mu(L_{(F,G)})$ and $\mu(R_{(F,G)})$ are equal to the desired iterated forms.
