...
% It often arises that two sets (properties) are to be inductively (or coinductively)
% defined \emph{simultaneously}, because each depends on the other.  Let $F$ be a monotone
% operator on $\wp{X}\times \wp{X}$ ordered componentwise by inclusion so that meets and joins are
% also given componentwise.  By arguments analogous to those given above, such an $F$ has a
% least and a greatest fixed point, $\mu(F)$ and $\nu(F)$, respectively.  Bekic's Lemma reduces
% this simultaneous inductive to a parameterized, iterated inductive definition.
% \begin{lemma}[Bekic]
%   For $F$ a monotone operator on $\wp{X}\times\wp{X}$, the least fixed point $\mu(F)$, given by the
%   meet of all pre-fixed points of $F$, is equal to the iterated fixed points 
% \end{lemma}
