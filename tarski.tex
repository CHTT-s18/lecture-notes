...
side conditions that constrain its applicability.  For example, in the typing rule for
...
definition of an assertion $A$ by the rule
\begin{displaymath}
  \inferrule
  {\neg A}
  {A}
\end{displaymath}
Were there a fixed point of the associated (non-monotone) operator, the judgment would be
derivable iff it is not, so obviously it must not have one.  More subtle examples are
observed in the wild, so be careful out there!
